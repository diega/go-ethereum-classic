...
func TestHandshake68PoW(t *testing.T) { testHandshake68PoW(t, ETH68) }

func testHandshake68PoW(t *testing.T, protocol uint) {
	t.Parallel()
	...
	for i, test := range tests {
		...
		err := peer.Handshake68PoW(1, big.NewInt(1), head.Hash(), genesis.Hash(), forkID, forkid.NewFilter(backend.chain))
		if err == nil {
